/**
 * Created by РумянцевДмитрий on 20.12.2017.
 */
import DBMain.StringCrypter;
...
public class CryptoUtil {
    ...
    public static void encryptIni(String name) throws IOException {
        String byteArrayAsString= "MoBI!!0}";
        StringCrypter crypter=new StringCrypter(byteArrayAsString.getBytes());
        FileWriter writer = new FileWriter("1_"+name, false);
        try {
            String testStr = readUsingFiles(name);
    ...
    public static void main(String[] args) {
        ...
        String testStr = null;
        try {
            encryptIni("options.ini");
        } catch (IOException e) {
            e.printStackTrace();
        }
        readIni rIni = new readIni();

        System.out.println(rIni.run("1_options.ini")[2]);
/*        try {
            testStr = readUsingFiles("options.ini");
        } catch (IOException e) {
            e.printStackTrace();
        }
        //"forCryptString"; */
...
// дешифрование
        ...
        System.out.println(encBase64Str);
